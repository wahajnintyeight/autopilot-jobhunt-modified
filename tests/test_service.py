...
def test_build_scheduler_defaults_include_apify_and_scan():
    ...
    assert set(jobs) == {"apify_scan_10h", "careers_scan_3h"}
    assert jobs["apify_scan_10h"].args[0].action == "apify_scan"
...
def test_build_scheduler_uses_cron_tasks():
    cfg = {
        "service": {
            "timezone": "UTC",
            "schedules": [
                {"name": "apify_10h", "action": "apify_scan", "cron": "0 */10 * * *"},
    ...
    assert set(jobs) == {"apify_10h", "morning_scan", "midday_export"}
    assert jobs["apify_10h"].args[0].action == "apify_scan"
